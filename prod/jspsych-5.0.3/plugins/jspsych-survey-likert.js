  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      for (var j = 0; j < trial.labels[i].length; j++) {
        options_string += '<li style="width:' + width + '%"><input type="radio" name="Q' + i + '" value="' + j + '"  style="height:35px; width:35px; vertical-align: middle;"><label class="jspsych-survey-likert-opt-label">' + trial.labels[i][j] + '</label></li>';
      }
      options_string += '</ul>';
